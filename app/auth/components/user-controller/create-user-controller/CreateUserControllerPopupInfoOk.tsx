...
interface CreateUserControllerPopupInfoOkProps {
  ...
  title?: string;
  username?: string;
  message?: string; // optional custom message when username not provided
  onClose: () => void;
}
        ...
        <View style={styles.card}>
          ...
          <Text style={styles.subtitle}>
            {username ? (
              <>
                {message ? (
                  message.replace("{username}", String(username))
                ) : (
                  <>
                    El usuario "<Text style={styles.bold}>{username}</Text>".
                  </>
                )}
              </>
            ) : (
