...
import { fetchEntradasFechaRaw, ApiEntradaFechaRaw, getTipoMap, fetchTiposEntrada, reservarEntradas, cancelarReserva, fetchReservaActiva, createPago } from "@/app/events/apis/entradaApi";
import { COLORS, FONT_SIZES, RADIUS } from "@/styles/globalStyles";
import { useAuth } from "@/app/auth/AuthContext";
import { getUsuarioById, updateUsuario } from "@/app/auth/userHelpers";
import InputText from "@/components/common/inputText";
import { fetchProvinces, fetchMunicipalities, fetchLocalitiesByName, fetchLocalities, fetchLocalitiesByProvince } from '@/app/apis/georefHelpers';
...
function BuyTicketScreenContent() {
  ...
  // Inicializar reservas activas si vienen desde la pantalla de evento (reservas pre-creadas)
  let initialReservas: string[] = [];
  try {
    if (typeof reservas === 'string' && reservas.length > 0) {
      initialReservas = JSON.parse(decodeURIComponent(reservas));
      if (!Array.isArray(initialReservas)) initialReservas = [];
    }
  } catch (e) {
    console.warn('[BuyTicketScreen] Error parsing initial reservas param:', e);
    initialReservas = [];
  }
  const [activeReservas, setActiveReservas] = useState<string[]>(initialReservas); // idCompra por cada fecha reservada
  ...
  // Si venimos desde EventScreen con reservas pre-creadas pero la 'selection' no llegó bien,
  // intentar popular selectedTickets a partir de la reserva activa del usuario.
  useEffect(() => {
    (async () => {
      try {
        // condiciones para intentar: tenemos eventData & entradasIndex cargado y no hay selección local
        const hasEntradasIndex = Object.keys(entradasIndex || {}).length > 0;
        const selEmpty = Object.keys(selectedTickets || {}).length === 0;
        if (!hasEntradasIndex || !selEmpty) return;
        if (!activeReservas || activeReservas.length === 0) return;

        const uid: string | null = (user as any)?.id ?? (user as any)?.idUsuario ?? null;
        if (!uid) return;

        // Traer reserva activa y mapear items (soportamos varias formas de respuesta)
        try {
          const reserva = await (await import('@/app/events/apis/entradaApi')).fetchReservaActiva(String(uid)).catch(() => null);
          if (!reserva) return;
          // reserva puede venir como objeto con items o como array de items
          const items: any[] = Array.isArray(reserva) ? reserva : Array.isArray((reserva as any).items) ? (reserva as any).items : [];
          if (!items || !items.length) return;

          const newSel: Record<string, number> = {};
          for (const it of items) {
            const tipo = it?.tipoEntrada ?? it?.tipo ?? it?.cdTipo ?? it?.cd_tipo ?? it?.cdTipoEntrada;
            const cantidad = Number(it?.cantidad ?? it?.qty ?? it?.cantidadEntradas ?? 0);
            const idFecha = it?.idFecha ?? it?.IdFecha ?? it?.fecha ?? it?.id_evento_fecha ?? null;
            if (!tipo || !idFecha || !cantidad) continue;

            // Buscar idEntrada en entradasIndex que coincida con idFecha + cdTipo
            const found = Object.values(entradasIndex).find((v) => String(v.idFecha) === String(idFecha) && Number(v.cdTipo) === Number(tipo));
            if (found && found.idEntrada) {
              newSel[`entrada-${found.idEntrada}`] = (newSel[`entrada-${found.idEntrada}`] || 0) + Number(cantidad);
            }
          }

          if (Object.keys(newSel).length > 0) {
            console.debug('[BuyTicketScreen] Poblando selectedTickets desde reserva activa:', newSel);
            setSelectedTickets((prev) => ({ ...newSel, ...prev }));
          }
        } catch (e) {
          console.warn('[BuyTicketScreen] Error poblando selección desde reserva:', e);
        }
      } catch (e) {
        /* ignore */
      }
  ...
  }, [eventData, entradasIndex, activeReservas]);
      ...
      try {
        ...
        for (const [idFecha, raw] of results) {
          for (let i = 0; i < raw.length; i++) {
            ...
            // Si el backend no provee idEntrada, generamos uno consistente con EventoPantalla
            // formato: gen-<idFecha>-<cdTipo>-<idx>
            const idEntrada = String(r.idEntrada ?? `gen-${idFecha}-${Number.isFinite(cd) ? cd : 0}-${i}`);
            const nombreTipo = (r?.tipo?.dsTipo && String(r.tipo.dsTipo)) || (Number.isFinite(cd) ? (tipoMap.get(cd) ?? "") : "") || "Entrada";
            const precio = Number(r?.precio ?? 0);
            // Indexamos siempre por idEntrada (existente o generado)
            if (idEntrada) {
  ...
  useEffect(() => {
    const sub = Linking.addEventListener("url", async (event: any) => {
      try {
        ...
        if (!url) return;
        // Log the raw incoming URL for debugging
        try { console.log('[BuyTicketScreen] deep link url:', url); } catch {}
        ...
        console.warn('[BuyTicketScreen] Linking event handler error:', e);
      }
    });
    return () => {
  ...
  const handleConfirmPurchase = async () => {
    ...
    try {
      ...
      // Before creating reservations, validate there's at least one ticket with qty > 0
      let anyPositive = false;
      for (const mapTipo of byFechaTipo.values()) {
        for (const qty of mapTipo.values()) {
          if (Number(qty) > 0) {
            anyPositive = true;
            break;
          }
      ...
      if (!anyPositive) {
        Alert.alert('Selección inválida', 'No se seleccionaron entradas con cantidad mayor a 0.');
        setIsSubmitting(false);
        return;
      }

      // Debug: log reservas por fecha (convert Maps to arrays for readability)
      try {
        const debugPayload = Array.from(byFechaTipo.entries()).map(([idF, m]) => [
          idF,
          Array.from(m.entries()),
        ]);
        console.debug('[BuyTicketScreen] Reservas por fecha (idFecha -> [cdTipo, cantidad]):', JSON.stringify(debugPayload));
      } catch {}

      let results: Array<{ idFecha: string; ok: boolean; idCompra?: string; error?: any }> = [];
      let collectedCompras: string[] = [];

      // If the Event screen already created reservations and passed them via params, use them
      if (activeReservas && activeReservas.length > 0) {
        console.debug('[BuyTicketScreen] Usando reservas pre-creadas desde pantalla de evento:', activeReservas);
        collectedCompras = [...activeReservas];
        results = collectedCompras.map((c) => ({ idFecha: '', ok: true, idCompra: c }));
      } else {
        // Realizar reservas desde esta pantalla (por fecha)
        for (const [idF, mapTipo] of byFechaTipo.entries()) {
          ...
          try {
            ...
            if (idCompra) {
              results.push({ idFecha: idF, ok: true, idCompra: String(idCompra) });
              collectedCompras.push(String(idCompra));
            } else {
              results.push({ idFecha: idF, ok: false, error: resp });
            }
          } catch (e) {
      ...
      // Verificar que la reserva exista en el servidor (prevención de casos donde el backend no persistió)
      try {
        const reservaAct = await fetchReservaActiva(String(uid));
        // Normalizar formas de respuesta del servidor: puede devolver
        // - un objeto { idCompra, items: [...] }
        // - o un array plano de items
        let hasItems = false;
        let reservaId: string | undefined;

        if (!reservaAct) {
          hasItems = false;
        } else if (Array.isArray(reservaAct)) {
          hasItems = reservaAct.length > 0;
          // intentar extraer idCompra del primer item si existe
          if (hasItems) reservaId = (reservaAct[0] as any)?.idCompra ?? (reservaAct[0] as any)?.IdCompra;
        } else if ((reservaAct as any).items && Array.isArray((reservaAct as any).items)) {
          hasItems = (reservaAct as any).items.length > 0;
          reservaId = (reservaAct as any).idCompra ?? (reservaAct as any).IdCompra;
        } else if ((reservaAct as any).idCompra) {
          // caso donde el API devuelve un objeto con idCompra pero sin items
          hasItems = true;
          reservaId = (reservaAct as any).idCompra;
        } else {
          // objeto no vacío -> aceptamos como válido
          try {
            hasItems = Object.keys(reservaAct as any).length > 0;
          } catch {
            hasItems = false;
          }
        }
        ...
        if (!hasItems) {
          // Intentar limpiar reservas locales que pudimos haber creado
          if (collectedCompras.length > 0) {
            for (const c of collectedCompras) {
              try {
                await cancelarReserva(String(c));
              } catch (e) {
                console.warn("cancelarReserva fallo durante rollback:", e);
              }
            }
            setActiveReservas((prev) => prev.filter((x) => !collectedCompras.includes(x)));
          }
          alert("No se pudo confirmar la reserva en el servidor. Intentá nuevamente más tarde.");
          return;
        }

        // Registrar la reserva activa en el estado local (si encontramos id)
        if (!reservaId) {
          if (!Array.isArray(reservaAct)) reservaId = (reservaAct as any)?.idCompra ?? (reservaAct as any)?.IdCompra;
        }
        if (reservaId) setActiveReservas((prev) => Array.from(new Set([...prev, String(reservaId)])));
      } catch (e) {
        console.warn("Error verificando reserva activa:", e);
        // No bloqueamos el pago por un fallo de verificación; seguimos pero informamos
        Alert.alert("Advertencia", "No se pudo verificar la reserva en el servidor. Si la compra falla, contactá soporte.");
      }
